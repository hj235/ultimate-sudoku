...
class StudentAgentPlayerTwo:
    ...
    def getLines(self, grid: np.ndarray) -> list:
        lines = [[]]*8
        for i in range(3):
            lines[i] = grid[i]
            lines[i+3] = grid.T[i]
    ...
    def getLinesImm(self, grid: np.ndarray) -> list:
        ...
        for i in range(3):
            ...
            lines[7][i] = grid[2-i][i]
        return lines
    def getDepthFromZeros(self, zeros: int) -> int:
        if zeros < 20:
            return 5
        if zeros < 15:
            return 6
        if zeros < 10:
            return 100
        return 4
    def countZeros(self, state: State):
        count = 0
        for i in range(3):
            for j in range(3):
    ...
    def globalScore(self, state: State):
        ...
        localBoardLines = self.getLinesImm(localBoardStatus)
        score = 0
    ...
    def choose_action(self, state: State) -> Action:
        ...
        # state = state.invert()
        # print("test")
        zeroCount = self.countZeros(state)
        depth = self.getDepthFromZeros(zeroCount)
...
class StudentAgent:
    def __init__(self, depth=4, sigmoidParam=3.0, stepBypassAmplifier=1.15):
        ...
        self.sigmoidParam = sigmoidParam
        self.stepBypassAmplifier = stepBypassAmplifier
        self.playerTwo = StudentAgentPlayerTwo()
    ...
    def choose_action(self, state: State) -> Action:
        ...
        movesPlayed = np.sum(state.board != 0)
        if movesPlayed % 2 == 1:
            return self.playerTwo.choose_action(state)
